-- Stage 4A repair: report cancelled destination jobs in the per-platform breakdown.
--
-- The Stage 4A RPC already returned a workspace-wide `publishing.cancelled`
-- count, but `platform_results` filtered cancelled jobs out entirely. That made
-- the platform breakdown disagree with the total publishing-job count whenever a
-- job was cancelled. Cancelled jobs remain excluded from the success-rate
-- denominator; they are neither successes nor failures.
--
-- Only the `platform_results` aggregate changes. Every other output key, the
-- signature, the security settings and the grants are unchanged.
...
begin
  ...
  -- Cancelled jobs are excluded from both sides of the success ratio.
  v_success_rate := case
  ...
  -- Changed in this migration: cancelled jobs are now carried through the
  -- per-platform breakdown so it reconciles with the total job count.
  ...
  from (
    ...
    where job.workspace_id = p_workspace_id
      and job.completed_at >= p_start_at and job.completed_at < p_end_at
      and job.status in ('succeeded', 'failed', 'reconciliation_required', 'cancelled')
      and (p_platform is null or job.platform = p_platform)

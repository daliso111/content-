...
export default function TeamPage() {
  ...
  return <div className="space-y-6">
    ...
    <InviteModal open={inviteOpen} roles={roles} workspaceId={activeWorkspace?.id ?? ""} onClose={() => setInviteOpen(false)} onInvited={async (delivery) => { setInviteOpen(false); toast.success("Invitation created", delivery === "email" ? "Supabase Auth was asked to send the invitation email." : "The existing PostFlow user received an in-app invitation."); await load(); }} />
    ...
    <ConfirmModal open={Boolean(pendingAction)} onClose={() => setPendingAction(null)} onConfirm={() => void confirmAction()} destructive={pendingAction?.kind === "remove"} confirmLabel={pendingAction?.kind === "transfer" ? "Transfer ownership" : pendingAction?.kind === "remove" ? "Remove member" : "Suspend access"}
      ...
      message={pendingAction?.kind === "transfer" ? `${pendingAction.member.fullName} becomes owner and your role becomes Administrator.` : pendingAction?.kind === "remove" ? "The membership is removed, but the PostFlow account and authored content are preserved." : "Access is removed immediately. Authored content remains in the workspace."} />
  </div>;
}
